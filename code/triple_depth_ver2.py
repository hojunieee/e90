...
tag_3d_estimation_path = os.path.join(save_folder, "triple_tag_estimations.csv")
...
def detect_tags(frame, camera_name, camera_matrix, dist_coeffs=None):
    ...
    for detection in detections:
        ...
        corners = detection.corners # 4 pixel coordinates, so (4, 2)
...
# ================= Use function to estimate positions & save 3d estimations in list ================= #
cam0_list = []
cam1_list = []
cam2_list = []
...
while True:
    ...
    cam0_result = detect_tags(frame0, "Cam 0", K0, None)
    cam1_result = detect_tags(frame1, "Cam 1", K1, None)
    cam2_result = detect_tags(frame2, "Cam 2", K2, None)

    if cam0_result and cam1_result and cam2_result:  # Ensure all cameras detected a tag
        cam0_list.append(cam0_result)
        cam1_list.append(cam1_result)
        cam2_list.append(cam2_result)
...
# ==================== Process 3D estimations using extrinsics & Save to CSV ==================== #

tag_3d_estimation_csv = []  

for i in range(len(cam1_list)):
    # Check if they have same length
    if len(cam0_list[i]) != len(cam1_list[i]) or len(cam0_list[i]) != len(cam2_list[i]):
        print(f"Frame {i} has inconsistent detection counts across cameras.")
        continue
    # Check if all cameras have detected a tag in this frame
    ...
    x_0_world = R_0 @ x_c0 + T_0              # convert to cam1 frame, shape
    ...
    x0, y0, z0 = cam0_pose
    x0_world, y0_world, z0_world = x_0_world.flatten()
    x1, y1, z1 = cam1_pose
    x2, y2, z2 = cam2_pose
    x2_world, y2_world, z2_world = x_2_world.flatten()

    tag_3d_estimation_csv.append([
        frame_indices[i],
        x0_world, y0_world, z0_world,
        x0, y0, z0,
        x1, y1, z1,
        x2, y2, z2,
        x2_world, y2_world, z2_world
    ])

# ================= SAVE DATA TO CSV (ONCE) ================= #
csv_filename = tag_3d_estimation_path
with open(csv_filename, "w", newline="") as csvfile:
    ...
    writer.writerows(tag_3d_estimation_csv) 
